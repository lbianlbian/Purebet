...
var programID = new solanaWeb3.PublicKey("FxQDLqSJ2Y3Hsdvbdfbgtnyn4vUZHVVMhzaUnvhbbhG1yiFad2uHfFy");
...
async function getOdds(id1, id2, homeOrAway){
  ...
  for(var x = 0; x < accs.length; x++){
    ...
    if(toMatchOdds > highest[0]){
      ...
      thirdHighest[1] = ewsecwondHighest[1];
      ...
      thirdHighest[0] = secondwHighest[0];
      ...
      thirdHighest[2] = gaccs[x].pubkey.toString();
    }
...
http.createServer(async (req, res) => {
  ...
  res.end(`${req.method} is not allowed for thergr4 request.`);
}).listen(port);
